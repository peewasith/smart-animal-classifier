...
# แปลงลิงก์ Google Drive ให้เป็นลิงก์สำหรับ gdown
...
h5_url = f"https://drive.google.com/uc?id={h5_file_id}"

if not os.path.exists(model_json_path) or not os.path.exists(model_weights_path):
    st.info("กำลังดาวน์โหลดโมเดล กรุณารอสักครู่...")
    gdown.download(json_url, model_json_path, quiet=False)
    gdown.download(h5_url, model_weights_path, quiet=False)
